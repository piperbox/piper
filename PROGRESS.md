...
- ✅ Docs organized by audience (`guides/`, `reference/`, `self-host/`, `ops/`) with `docs/manifest.json` as the publish list and a `test/docs` contract test — [spec](docs/superpowers/specs/2026-09-11-docs-organization-design.md); the dashboard sync follows
...
- ✅ `piper-relay` store on Postgres — one shared database for every relay process; SQLite removed from the relay; `PIPER_RELAY_DB_URL` required — [#515](https://github.com/piperbox/piper/issues/515)
- ✅ `piper-edge` + tunnel ownership — owner-routed L4 entrypoint in front of N relays (`relay_instances`/`agent_owners`, LISTEN/NOTIFY, control hop, NOTIFY-woken webhook drain); login-flow state and graceful drain tracked in the epic — [#521](https://github.com/piperbox/piper/issues/521) (child of epic [#524](https://github.com/piperbox/piper/issues/524))
...
- ✅ graceful relay drain — SIGTERM flips the pool row to `draining` (edge places nothing new there, keeps routing what it owns), refuses `:7000`, closes each session once its streams end (20 s bound), then leaves the pool; zero-drop landed with [#530](https://github.com/piperbox/piper/issues/530) — [#523](https://github.com/piperbox/piper/issues/523) (child of epic [#524](https://github.com/piperbox/piper/issues/524))
- ✅ two tunnel sessions per agent — the handshake carries a routing preface the edge peeks; `:7000` places away from the agent's current owners; `agent_owners` is one row per session and `:443`/`:80` pick among owners and retry once; relays refuse duplicates after auth — pinging the held session first and replacing it when half-open, [#538](https://github.com/piperbox/piper/issues/538) — and derive routes from Postgres on `piper_hostnames`; a relay going away is never an agent's only link — [#530](https://github.com/piperbox/piper/issues/530) (child of epic [#524](https://github.com/piperbox/piper/issues/524))
- ✅ e2e on the supported relay topology — every relay e2e runs `piper-edge` + two `piper-relay` (two zones, shared Postgres) from one harness helper; `TestRelayCluster` proves two owners, serving through a drained relay, and re-placement on the rolled relay's return — [#537](https://github.com/piperbox/piper/issues/537) (child of epic [#524](https://github.com/piperbox/piper/issues/524))
- ✅ slot 1 boot race — the duplicate wait is a ladder (2 s → ×4 → 1 min), so a slot 1 the edge placed before the owner row landed is back in seconds instead of a minute; found by the #537 harness, seen in production — [#548](https://github.com/piperbox/piper/issues/548)
- ✅ Kubernetes rollout readiness — `/readyz`+`/livez` on both ops listeners, edge drain on SIGTERM, advisory-locked schema apply, runbook Kubernetes subsection (non-root stays a `runAsUser` recipe: [#554](https://github.com/piperbox/piper/issues/554)) — [#557](https://github.com/piperbox/piper/issues/557)
...
- ✅ **Public-relay onboarding slice (Plans 1–3)** — relay accounts + device-flow, `piper login`/`connect`, and relay-terminated shared domain; `login → connect → deploy → curl` e2e green — [#90](https://github.com/piperbox/piper/issues/90) (child of epic [#49](https://github.com/piperbox/piper/issues/49))
  ...
  - ✅ Direct serve mode: box-wide custom domain served by the box's own :443 (DNS → box), relay kept for login/webhooks/control — [#505](https://github.com/piperbox/piper/issues/505)
